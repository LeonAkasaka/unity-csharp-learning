...
> 💡 **ポイント**: `Time.deltaTime` を掛けた場合のもう一方の数値（ここでは `3.0f`）は「**1秒あたりの移動量（速度）**」を意味します。`Time` クラスには他にも時間管理に役立つプロパティがあります。詳細は [Time クラスと時間制御](/unity-csharp-learning/unity/time-basics/) で扱います。
