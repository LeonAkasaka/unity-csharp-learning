...
ダウンロード ページに移動するので「SDユニティちゃん 3Dモデルデータ」をダウンロードしてください。本校では Version 1.1.0 を想定しています。
...
ウィンドウ左上の再生ボタンを押すと、現在設定されているアニメーションクリップをプレビュー再生を行います。
...
以降、animator 変数を通して Animator コンポーネントを操作できるようになります。Update() メソッド内で Play() メソッドを呼び出し "Running@loop" アニメーションを再生するように
...
> **注意: 旧 Input システム**  
> 以下のコードは旧 Input System（`Input.GetKey`）を使用しています。Unity 6 では新しい Input System が推奨されますが、旧 Input System も引き続き使用できます。詳しくは [旧 Input](/unity-csharp-learning/unity/legacy-input/) を参照してください。  
...
Input クラスの GetKey() メソッドは上キーを押している間 true を返すので、キーが押されている間、連続で Play() メソッドを呼び出すことになりますが、同じ状態のアニメーションの再生はリセットされることなく維持されるので問題ありません。
