...
- Animator Controller を使ってアニメーションの状態遷移を理解できる
- スクリプトから Animator パラメータを設定してアニメーションを切り替えられる
...
ダウンロード ページに移動するので「ユニティちゃん 3Dモデルデータ」をダウンロードしてください。本稿では Version 1.4.0 を想定しています。

![](./image-2.png)

「UnityChan_v1.4.0.unitypackage」という Unity パッケージファイルがダウンロードされます。
...
ゲームを起動すると、この Animation Controller は既定の状態として Idle 状態になります。状態は再生するべきアニメーションを保持しています。Idle 状態をクリックして Inspector ビューを確認してください。この状態で再生するアニメーションの設定が含まれています。
...
遷移条件は、指定のパラメータが指定の条件を満たしているかどうかを表します。上記の例では「Speed パラメータが 0.1 より大きければ」 Idle 状態から Locomotion 状態に遷移することを表します。では、Speed パラメータとは何でしょう。

Animator Controller は制作者が任意のパラメータを持たせることができ、これを制御することで状態を切り替えられます。パラメータの確認や編集を行うには Animator ビューの「Parameters」タブを選択してください。

![](./image-21.png)

ここに定義されたアニメーション用のパラメータと、その初期値が設定されています。パラメータの種類（型）には整数（Int）、実数（Float）、チェック（Bool）などがあります。

この Parameters タブの一覧に Speed パラメータが定義済みであることに注目してください。Speed パラメータは実数型で初期値は 0.0 になっています。ここで Idle 状態から Locomotion 状態に遷移する条件を思い出しましょう。Speed パラメータが 0.1 より大きければ遷移する条件なので、この値を 0.1 より大きな値にすれば状態が Locomotion に移動し、再生するアニメーションが変化します。
...
[Input System](/unity-csharp-learning/unity/input-system/) の `Keyboard.current` を使ってキー入力を検知します。↑ キーを押している間は Speed パラメータを 1.0 に、↓ キーを押している間は -1.0 に、どちらも押していない場合は 0.0 に設定します。
...
↑ キーを押すと Speed パラメータが 1.0 になるので、連動してユニティちゃんが走るアニメーションに切り替わります。↓ キーを押すと Speed パラメータが -0.1 より小さくなるので Idle 状態から WalkBack 状態への遷移条件が満たされ、ユニティちゃんが後退するような歩行アニメーションが再生されます。

## 課題

### 課題1 左右の傾きモーションを追加しよう

左右の矢印キーに対応して左右に曲がるアニメーションを再生できるようにしましょう。UnityChanLocomotions に Direction パラメータが定義されているので、このパラメータを -1.0 ～ 1.0 の範囲で設定すると左右に傾いて歩行または走るようになります。`Keyboard.current.leftArrowKey.isPressed` と `Keyboard.current.rightArrowKey.isPressed` を使って入力を取得してください。
...
- **Animator Controller** は状態マシンとしてアニメーションの遷移を管理します
- **アニメーション状態**（State）には再生する Animation Clip が設定され、**遷移**（Transition）の条件を満たすと次の状態に移行します
- スクリプトから `Animator.SetFloat()` / `SetInteger()` / `SetBool()` でパラメータを操作することで、アニメーションをプログラム的に制御できます
- `Keyboard.current` を使ってキー入力を検知し、Animator パラメータに反映することでキャラクターを操作できます
