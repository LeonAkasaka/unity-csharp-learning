...
中間言語が存在する主な理由は**プラットフォームの違いを吸収するため**です。Windows・macOS・Linux・iOS・Android はそれぞれ異なる実行環境であり、独自の命令セットを持ちます。

![プラットフォームごとのアプリケーション開発](image.png)

上記のようにプログラムコードを機械語にコンパイルして実行する場合、プラットフォームごとに別々のコンパイルが必要になります。端的にいえば、Windows アプリケーションを MacOS で動かすことはできません。これは、プログラムで書かれたコードを実行する命令セット（ハードウェアや OS が提供する機能）が異なるためです。

しかし .NET はハードウェアと OS を抽象化するランタイムと呼ばれるソフトウェアが仮想的な実行環境となってアプリケーションを実行します。このランタイム用の独自の言語が中間言語です。

![.NET アプリケーション実行の仕組み](image-1.png)

中間言語を挟むことで、**C# の変換は1回**で済み、各プラットフォームへの機械語への変換はランタイム側に任せられます。
